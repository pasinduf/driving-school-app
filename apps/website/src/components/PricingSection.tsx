...
const PricingCard = ({ pkg }: { pkg: PackageData, index: number }) => {
    return (
      <div
        className={`relative flex flex-col h-full bg-white rounded-3xl overflow-hidden transition-all duration-300 transform hover:-translate-y-2 ${
          pkg.isHighlight ? "ring-2 ring-primary shadow-2xl scale-105 z-10" : "shadow-lg hover:shadow-xl border border-gray-100"
        }`}
      >
        {pkg.isHighlight && (
        ...
        <div className={`pt-10 pb-8 px-8 text-center ${pkg.isHighlight ? "bg-red-50/30" : ""}`}>
          <h3 className={`font-black text-xs uppercase tracking-[0.2em] mb-4 ${pkg.isHighlight ? "text-primary" : "text-gray-400"}`}>{pkg.name}</h3>
        ...
        <div className="flex-1 flex flex-col px-8 pb-8">
          <div className="space-y-4 mb-8">
            <p className="text-gray-600 text-sm leading-relaxed text-center font-medium italic">"{pkg.description}"</p>

            <div className="h-px w-12 bg-gray-100 mx-auto"></div>

            <ul className="space-y-3 pt-2">
              {["Professional Instructor", "Progress Tracking"].map((feature, i) => (
                <li key={i} className="flex items-center gap-3 text-gray-600">
                  <div
                    className={`flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center ${pkg.isHighlight ? "bg-primary/10 text-primary" : "bg-gray-100 text-gray-400"}`}
                  >
          ...
          <a
            href={bookingUrl()}
            className={`mt-auto w-full py-4 text-center rounded-2xl font-bold uppercase tracking-widest text-xs transition-all duration-300 ${
              pkg.isHighlight
